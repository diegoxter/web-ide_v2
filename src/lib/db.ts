...
export async function addDirectory(db: IDBDatabase, directoryName: string): Promise<boolean> {
...
export async function addFile(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      console.log(`Archivo "${fileName}" añadido correctamente al directorio ${directoryId}.`);
...
export async function listDirectoriesWithFiles(db: IDBDatabase): Promise<DBDirectory[]> {
  return new Promise((resolve, reject) => {
    ...
    dirRequest.onsuccess = (event) => {
      ...
      if (cursor) {
        ...
        fileRequest.onerror = (event) => {
          console.error("Error al recuperar archivos:", (event.target as IDBRequest).error);
    ...
    dirRequest.onerror = (event) => {
      console.error("Error al listar los directorios:", (event.target as IDBRequest).error);
      reject((event.target as IDBRequest).error);
    };
  });
}
